...
def aug_data(x, y):
    images = tf.identity(x)
    labels = tf.identity(y)
    ...
    with tf.name_scope('augument'):
        ...
        with tf.name_scope('random_crop'):
            ...
            images = tf.map_fn(lambda i: tf.random_crop(i, size=[config.IMG_SIZE, config.IMG_SIZE, 1], seed=seed), images)
            labels = tf.map_fn(lambda i: tf.random_crop(i, size=[config.IMG_SIZE, config.IMG_SIZE, 1], seed=seed), labels)
        ...
        # with tf.name_scope('gaussian_noise'):
            ...
            #     shape=[batch_size, config.IMG_SIZE, config.IMG_SIZE, 1],
            #     mean=0.0,
            #     stddev=0.02,
            #     dtype=tf.float32
            # )
